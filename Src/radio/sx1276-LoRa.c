...
tLoRaSettings LoRaSettings =
{
    ...
    0,            // ImplicitHeaderOn [0: OFF, 1: ON] 【0：显示报头， 1：隐式报头】
    1,                // RxSingleOn [0: Continuous, 1 Single]
    0,                // FreqHopOn [0: OFF, 1: ON]
    4,                // HopPeriod Hops every frequency hopping period symbols
    100,              // TxPacketTimeout
    LORA_BREATH_TIME,              // RxPacketTimeout
    255,              // PayloadLength (used for implicit header mode)
};

const float LORA_SignalBw[11] = {7.8, 10.4, 15.6, 20.8, 31.2, 41.6, 62.5, 125, 250, 500, 500};
const uint16_t LORA_SpreadingFactor[8] = {64, 128, 256, 512, 1024, 2048, 4096, 4096};

#define LORA_DIO_ENABLE 0             //为0不是用DIO0~DIO5
...
uint8_t RF_StartAddr[4];

uint16_t PreambleLength;

uint32_t Tick_prev = 0;
uint32_t TickTmp = 0;
uint8_t ValidheaderDetected = 0;
uint8_t RxHeaderNb = 0;
/*!
...
#if 0	
		PreambleLength = ((LORA_SLEEP_TIME + 2 * LORA_BREATH_TIME) * RF_Bw / RF_Sf) - 4;    //  2064 * 125 / 128 - 4 = 1994
		SX1276LoRaSetPreambleLength(PreambleLength);                //最大：0xFFFF
		RFDebug("set PreambleLength = %d\r\n",PreambleLength);	
#endif

		PreambleLength = Preamble_Default;                                  //默认的前导码符号个数：8 symbols 最大：0xFFFF
		SX1276LoRaSetPreambleLength(PreambleLength);                
		printf("set PreambleLength = %d\r\n",PreambleLength);	

		SymbTimeout = (LORA_BREATH_TIME) * RF_Bw / RF_Sf + 1;
		if(SymbTimeout < 4)                 // 最少 4
			SymbTimeout = 5;
		else if(SymbTimeout > 0x3ff)        // 最大0x3FF
			SymbTimeout = 0x3ff;
		printf("set SymbTimeout = %d\r\n",SymbTimeout);
		SX1276LoRaSetSymbTimeout(SymbTimeout);
//		SX1276LoRaSetSymbTimeout( 0x3ff);  // 最大0x3FF
...
void SX1276LoRaGetRegsValue( void )
{
	        SX1276ReadBuffer( REG_LR_OPMODE, SX1276Regs + 1, 0x70 - 1 );
...
//		RFDebug("SX1276 REG_LR_VERSION:0x%02x!\r\n",SX1276LR->RegVersion);
}
...
void SX1276LoRaGetRxPacketData( void *buffer, uint16_t *size )
{
    *size = RxPacketSize;
    RxPacketSize = 0;
    memcpy( ( void * )buffer, ( void * )((uint8_t *)RFBuffer + 4), ( size_t )*size - 4 );
}
...
//#if 0
//		RF_Bw= 125;                                                 // LoRaSettings.SignalBw = 7            : 125khz
//		RF_Sf= 1 << LoRaSettings.SpreadingFactor;                   //	LoRaSettings.SpreadingFactor = 7     : 128
//		PreambleLength = (RF_Preamble_Time * RF_Bw / RF_Sf) - 4;    //  2064 * 125 / 128 - 4 = 1994
//		SX1276LoRaSetSymbTimeout( (LORA_BREATH_TIME) * RF_Bw / RF_Sf + 1);  // 32ms * 125khz / 128 + 1=  32 ,最大0x3ff
//    RFDebug("set SymbTimeout = %d\r\n",(LORA_BREATH_TIME) * RF_Bw / RF_Sf);
//#else
//		PreambleLength = 0x10;
//		SX1276LoRaSetPreambleLength(PreambleLength);                //最大：0xFFFF
//		SX1276LoRaSetSymbTimeout( (LORA_BREATH_TIME) * RF_Bw / RF_Sf + 1);  // 32ms * 125khz / 128 + 1=  32 ,最大0x3ff
...
//#endif
        ...
        RxTimeoutTimer = GET_TICK_COUNT( );
				...
				#if(JOHN_DEBUG)
				Tick_prev = GET_TICK_COUNT( );
				RFDebug("\r\nRx Start:%d",Tick_prev);
				#endif
        break;
    case RFLR_STATE_RX_RUNNING:     
        ...
        if( DIO2 == 1 ) // FHSS Changed Channel
        {
            ...
            RxGain = SX1276LoRaReadRxGain( );
				...
				if( DIO3 == 1 ) //ValidHeader Detected
				{
					#if(JOHN_DEBUG)
					TickTmp = GET_TICK_COUNT( );
					RFDebug("\r\n ValidHeader +%dms!\r\n",TickTmp);
					#endif
					result = RF_VALIDHEADER_DETECTED;
					SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
				}
				if( DIO4 == 1 ) // CadDetected
        {
					#if(JOHN_DEBUG)
					TickTmp = GET_TICK_COUNT( );
					RFDebug("\r\n CadDetected +%dms!\r\n",TickTmp);
				...
				if(SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_RXDONE)              //RXDONE
				{
				 // Clear Irq
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE  );
            RFLRState = RFLR_STATE_RX_DONE;
						if( LoRaSettings.FreqHopOn == true )
							...
							#if(JOHN_DEBUG)
							TickTmp = GET_TICK_COUNT( )  - Tick_prev;
							RFDebug("\r\nRX_DONE +%dms!",TickTmp);
							#endif
				}
				if(SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_RXTIMEOUT)        //RXTIMEOUT
				{
				 // Clear Irq
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXTIMEOUT  );
            RFLRState = RFLR_STATE_RX_TIMEOUT;
					#if(JOHN_DEBUG)
						TickTmp = GET_TICK_COUNT( )  - Tick_prev;
						RFDebug("\r\nIRQ_RXTIMEOUT +%dms!",TickTmp);
					#endif
				}
				if(SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_CADDETECTED)      // CADDETECTED
				{
				 // Clear Irq
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_CADDETECTED  );
            result = RFLR_STATE_RX_INIT;
					#if(JOHN_DEBUG)
					TickTmp = GET_TICK_COUNT( )  - Tick_prev;
					RFDebug("\r\nCADDETECTED +%dms!\r\n",TickTmp);
					#endif
				}
				if(SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL) // FHSS Changed Channel
        {
					#if(JOHN_DEBUG)
					TickTmp = GET_TICK_COUNT( )  - Tick_prev;
						RFDebug("\r\n FHSS Changed +%dms!\r\n",TickTmp);
            ...
            RxGain = SX1276LoRaReadRxGain( );
				...
				if(SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_VALIDHEADER) //ValidHeader Detected
				{
					#if(JOHN_DEBUG)
					TickTmp = GET_TICK_COUNT( )  - Tick_prev;
					RFDebug("\r\n ValidHeader +%dms!",TickTmp);
					#endif
					ValidheaderDetected = 1;
					SX1276LR->RegFifoAddrPtr = SX1276LR->RegFifoRxBaseAddr;
          SX1276Write( REG_LR_FIFOADDRPTR, SX1276LR->RegFifoAddrPtr );
					
					SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
				}
#if(VALID_HEADER_DETECTED)		
				if(ValidheaderDetected == 1)
				{
					SX1276Read( REG_LR_FIFORXBYTEADDR, &RxHeaderNb );
					if(RxHeaderNb >= 4 )
					{	
						RxHeaderNb = 0;
						ValidheaderDetected = 0;
						SX1276ReadFifo(RF_StartAddr, 4 );
						RFDebug("\r\n RF_StartAddr = %s \r\n ",RF_StartAddr);
						if(RF_StartAddr[0] != NetAddr[0] || RF_StartAddr[1] != NetAddr[1] || RF_StartAddr[2] != NodeAddr[0] || RF_StartAddr[3] != NodeAddr[1])
						{
							result = RF_RX_TIMEOUT;
							RFLRState = RFLR_STATE_IDLE;
							#if(JOHN_DEBUG)
							TickTmp = GET_TICK_COUNT( )  - Tick_prev;
							RFDebug("\r\nAddress error +%dms!",TickTmp);
							#endif
						}	
					}	
				}
...
//					} 		
    ...
    case RFLR_STATE_RX_DONE:
        ...
        /*       获取接受数据后的信噪比： SNR = REG_LR_PKTSNRVALUE 取补码后剩4*/
        {
        ...
        else // Rx continuous mode
        {
			...
			//			RFDebug("\r\nRx continuous mode RegFifoRxCurrentAddr = %d\r\n",SX1276LR->RegFifoRxCurrentAddr);
            if( LoRaSettings.ImplicitHeaderOn == true )
            {
                RxPacketSize = SX1276LR->RegPayloadLength;
    ...
    case RFLR_STATE_TX_INIT:
        ...
        SX1276WriteFifo( RFBuffer, SX1276LR->RegPayloadLength );
                                        // TxDone               RxTimeout                   FhssChangeChannel          CadDone         
        SX1276LR->RegDioMapping1 = RFLR_DIOMAPPING1_DIO0_01 | RFLR_DIOMAPPING1_DIO1_00 | RFLR_DIOMAPPING1_DIO2_00 | RFLR_DIOMAPPING1_DIO3_00;
                                        // PllLock              Mode Ready
...
uint16_t GetPreambSymbols(uint16_t PreambleTime)
{
	return ((PreambleTime * RF_Bw / RF_Sf) - 4);
}
